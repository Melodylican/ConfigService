...
<body>
						...
						<p class="fl">
							<label>游戏名称</label> <select id="u4_input" name="searchGameName">
								<c:forEach items="${gameNameList}" var="nameList">
									<option
										<c:if test="${currentGameName==nameList }">selected="true"</c:if>
			...
			<div class="wrap">
					...
					<table width="1000">
						...
						<tbody>
							...
							<tr class="odd">
								...
								<td>
									<label>否</label>&nbsp;&nbsp;&nbsp; <a id="detail" data-reveal="detail" href="javascript:void(0);" title="点击查看积分详情" item-id1="${list.playerId}" item-id2="${list.gameId}" >积分详情</a>
								</td>
								<td>
								<c:choose>
								   <c:when test="${list.status == 'PASSED' }">
							             <a href="<%=request.getContextPath()%>/promoter/throughaudit?orderId=${list.orderId }&page=${page}&pageSize=${pageSize}&searchGameName=${currentGameName}" title="点击通过审批" style="background-color:green">通过</a>
							       </c:when>
							       <c:otherwise>
							             <c:if test="${list.status == 'REJECTED' }">
							               <a href="javascript:return false;" title="未通过订单不能再次通过审批" class="disabled" style="background-color:#ccc">通过</a>
							             </c:if>
							             <c:if test="${list.status ne 'REJECTED'}">
							               <a href="<%=request.getContextPath()%>/promoter/throughaudit?orderId=${list.orderId}&state=${list.status }&page=${page}&pageSize=${pageSize}&searchGameName=${currentGameName}" title="点击通过审批" >通过</a>							             
							             </c:if>
							       </c:otherwise>
							    </c:choose>
							    <c:choose>
								   <c:when test="${list.status == 'REJECTED' }">
							             <a href="javascript:void(0);" id="notpass" item-id2="${page}" item-id3="${pageSize}" item-id="${list.orderId}" item-id1="${currentGameName}" data-reveal-id="myModal1" data-animation="fade" title="点击未通过审批" style="background-color:#206ecd">未通过</a>
							       </c:when>
							       <c:otherwise>
							             <a href="javascript:void(0);" id="notpass" item-id2="${page}" item-id3="${pageSize}" item-id="${list.orderId}" item-id1="${currentGameName}" data-reveal-id="myModal1" data-animation="fade" title="点击未通过审批">未通过</a>
							       </c:otherwise>
							    </c:choose>
							    <c:choose>
				...
				<div class="tfoot"> 
				              <form action="<%=request.getContextPath()%>/promoter/pass" method="POST">
				                   ...
				                   <input name="searchGameName" hidden="hidden" value="${currentGameName}">
				                   <input type="submit" value="批量通过" id="pass" class="btn"/>
				              </form>
			   </div>
				<div class="pagination">
				    <c:if test="${pages>1 && page>1 }">
                             <a href="<%=request.getContextPath()%>/promoter/orderajax?page=${page-1}&pageSize=${pageSize}&searchGameName=${currentGameName}">上一页</a>
                    </c:if>
				           ...
				           <c:otherwise>
				                ...
				                <c:otherwise>
				                   <c:if test="${index==pages }"> 
				                     <a href="<%=request.getContextPath()%>/promoter/orderajax?page=${index}&pageSize=${pageSize}&searchGameName=${currentGameName}" <c:if test="${page ==pages  }" >class="cur"</c:if>>${pages}</a>
				                   </c:if> 
				                </c:otherwise>
				                </c:choose>
				           </c:otherwise>
                    ...
                    <c:if test="${pages>1 && page < pages }">
                             <a href="<%=request.getContextPath()%>/promoter/orderajax?page=${page+1}&pageSize=${pageSize}&searchGameName=${currentGameName}">下一页</a>
                    </c:if>
				</div>
			</div>
		</div>
	...
	<div id="myModal" class="reveal-modal">
			<center><h1>您确定要作废该条信息吗？<br><font color="red">(当前该功能未启用)</font></h1></center><br><br>
