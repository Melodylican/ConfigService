			...
			<div class="wrap">
						...
						<tbody>
							...
							<tr>
							    <td colspan="8" align="left">
							           <p align="left"><a href="<%=request.getContextPath()%>/promoter/export?searchGameName=${currentGameName}" style="background-color:green" id="delete">导出Excel</a></p>
							    </td>
							</tr>
							</c:if>
						</tbody>
				...
				<div class="pagination">
				    <c:if test="${pages>1 && page>1 }">
                             <a href="<%=request.getContextPath()%>/promoter/orderpay?page=${page-1}&pageSize=${pageSize}">上一页</a>
                    </c:if>
				           ...
				           <c:when test="${(index-page< 0 ? page-index:index-page)< 5 }">
				               <c:choose>  
                                  <c:when test="${index==page }">
                                        <a href="<%=request.getContextPath()%>/promoter/orderpay?page=${index}&pageSize=${pageSize}" class="cur">${index}</a>
                                  </c:when>  
                                  <c:otherwise>
                                        <a href="<%=request.getContextPath()%>/promoter/orderpay?page=${index}&pageSize=${pageSize}" >${index}</a>
                                  </c:otherwise>  
                               </c:choose>
				           </c:when>
				           <c:otherwise>
				                ...
				                <c:otherwise>
				                     ...
				                     <a href="<%=request.getContextPath()%>/promoter/orderpay?page=${index}&pageSize=${pageSize}"  <c:if test="${page ==pages  }" >class="cur"</c:if>>${pages}</a>
				                    </c:if>
				                </c:otherwise>
				                </c:choose>
				           </c:otherwise>
                    ...
                    <c:if test="${pages>1 && page < pages }">
                             <a href="<%=request.getContextPath()%>/promoter/orderpay?page=${page+1}&pageSize=${pageSize}">下一页</a>
                    </c:if>
				</div>
		</div>
	</div>
