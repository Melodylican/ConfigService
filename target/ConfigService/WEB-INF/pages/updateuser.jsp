...
<body>
	...
	<div class="mainWrap clearfix">
		...
		<div class="colMain">
			<div class="crumb">
				<span>当前位置：</span> <strong>积分配置管理 > 兑换码管理          <c:if test="${not empty updateMsg}">
				                                              <h1><font color="red">${updateMsg}</font></h1>
			                                           </c:if></strong>
			</div>
			<div class="wrap">
				<div class="grid">
					<form id="Form" action="<%=request.getContextPath()%>/user/saveupdate"  modelAttribute="userBean" method="POST">
						<fieldset
							...
							<c:forEach items="${list}" var="list">
								<c:choose>
									<c:when test="${fn:contains(userBean.gameType, list.gameName)}">
										<input type="checkbox"  value="${list.gameName }"   checked="checked" class="inputStyle"/>${list.gameName }
									</c:when>
									<c:otherwise>
                                    ...
                                    <p><label></label>
                                       <textarea name="gameType"  placeholder="拥有操作游戏的权限..." readonly="readonly" style="width:250px;height:60px;padding:5px;border:1px solid #c9c9c9;color:#666;background-color:#dcdcdc" id="gamedate" >${userBean.gameType}</textarea>
                                    </p>
							<p>
								<label>账号用途简述</label>
								<c:choose>
